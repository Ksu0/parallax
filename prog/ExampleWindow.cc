...
ExampleWindow::ExampleWindow(int width, int height) :
	...
	_city = std::shared_ptr<SDL_Texture>(
				IMG_LoadTexture(_renderer.get(), "city.png"), SDL_DestroyTexture);
		if (_city == nullptr)
			throw std::runtime_error(
					std::string("Не удалось загрузить город: ")
					+ std::string(SDL_GetError()));
...
void ExampleWindow::render() {
	...
	SDL_Rect mountes_rect { _sky_x, _city_y, width(), height() / 2 };
	SDL_Rect mountes_rect2 { _sky_x2, _city_y, width(), height() / 2 };
	...
	SDL_RenderCopy(_renderer.get(), _city.get(), nullptr, &mountes_rect);
	SDL_RenderCopyEx(_renderer.get(), _city.get(), nullptr, &mountes_rect2, 0, nullptr, SDL_FLIP_HORIZONTAL);
